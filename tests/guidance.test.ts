...
describe("reliability documentation contract", () => {
  it("keeps GG-BL-010 mitigated until the bounded cross-client gate passes", () => {
    ...
    expect.soft(incident).toMatch(/\*\*Status\*\*: MITIGATED/);
    expect.soft(incident).toMatch(/\*\*Done when\*\*:[\s\S]*(跨客户端|cross-client)[\s\S]*(两任务|two-task|两个.*用户任务)/i);
