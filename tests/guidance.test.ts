...
describe("reliability documentation contract", () => {
  it("keeps the GG-BL-010 formal evidence while recording the explicit owner-risk closure", () => {
    ...
    expect.soft(incident).toMatch(/status:\s*done/);
    expect.soft(incident).toContain("3/3 PASS");
    expect.soft(incident).toContain("owner-accepted residual risk");
    expect.soft(incident).toContain("未按原 frozen-identity 条件完整证明");
    expect.soft(incident).toContain("DONE date");
