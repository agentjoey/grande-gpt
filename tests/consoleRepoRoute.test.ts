...
describe.skip("POST /console/repos/:repoId/register", () => {
  it("空目录经 Gateway 一次完成最小 Git 初始化 + canonical registration", async () => {
    ...
    try {
      ...
      const body = await res.json() as { ok: boolean; data: { repoId: string; initialized: boolean; registered: boolean } };
      expect(body).toEqual({ ok: true, data: { repoId: "empty-project", initialized: true, registered: true } });
  ...
  it("非空非 Git 目录 readiness 失败时 409 且不注册", async () => {
    ...
    try {
      ...
      expect(body.error.message).toMatch(/Git repository|readiness/);
    } finally {
      db.close();
    }
  });
