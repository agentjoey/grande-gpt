...
export function buildProfile(p: SandboxPaths): string {
  ...
  return [
    ...
    ";; worktree 本身也要放行 exec——U2 实测：pnpm/npm 把包的可执行入口（如",
    ";; node_modules/.bin/vitest）生成为物理落在 worktree 内的 POSIX shell shim（不是",
    ";; 符号链接出去），`pnpm test` 经由该 shim 的 shebang 调起，因此 shim 自身必须可 exec。",
    ";; 这是真实的安全取舍：worktree 装的是不可信仓库内容。但同一条 `pnpm test` 命令本来就会",
    ";; 把仓库自带的测试/构建脚本当代码跑（vitest.config.ts、setup 文件等已经是任意代码执行），",
    ";; 这条 allow 并未新增可达的攻击面，只是把「不可信代码被执行」这件事从「脚本内容」",
    ";; 延伸到「脚本文件本身的 exec 位」，边界不变。",
    `(allow process-exec (subpath "${q(p.worktree)}"))`,
    "(allow process-fork)",
    "(allow sysctl-read)",
    "",
  ].join("\n");
