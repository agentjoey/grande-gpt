...
export function renderMarkdown(a: Analysis, logPath: string | null = null): string {
  ...
  const lines: string[] = [
    ...
    "> **上表的间隔判定只是弱证据。** 强证据是 ChatGPT 每个回合顶部的 `Worked for Xs`：",
    "> 一段轮询链若整体落在**单个回合**内，人在结构上就无法插话——这比任何间隔阈值都硬。",
    "> 因此请对照回合边界核验：某个 episode 的全部轮询是否都在同一回合内完成。",
    "> 若某段轮询跨了回合，说明中间有人发过消息，该 episode 的「自主」判定不成立。",
    "",
    "## P-2 额度消耗",
    "",
