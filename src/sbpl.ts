...
export function buildProfile(p: SandboxPaths): string {
  ...
  return [
    ...
    "(deny network*)",
    "",
    ";; /dev/null /dev/urandom /dev/random —— git 与大量标准 Unix 工具的基础 I/O 机制",
    ";; git 打开 /dev/null 抑制信息输出（例如 `git status --short` 的重定向），",
    ";; OpenSSL/libcrypto 用 /dev/urandom 作为 CSPRNG 熵源，/dev/random 同理。",
    ";; 用 literal 不用 subpath——对整个 /dev 做 subpath 匹配会把整个设备树（磁盘、pty、",
    ";; dtrace 等）放行进来，只有这三个字符设备是 git 与大多数 Unix 工具实际需要的",
    ";; 最小集合。",
    ";;",
    ";; /dev/null 需要读写：git 会以 O_RDWR 打开它（例：内部 pipeline 的 dup2）。",
    ";; /dev/urandom 与 /dev/random 只需要读。",
