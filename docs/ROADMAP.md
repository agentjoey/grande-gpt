...
- Phase 8 closeout 当时的 production tool identity 为 25 tools / epoch 2 / 原 digest。
...
**Status**：IN PROGRESS（Minimal V2 slice 已于 2026-09-08 production activation；其余 surface convergence 未完成）
**范围**：`GG-BL-024`。
原目标是在一次正式 Tool Epoch 中收敛公开 MCP surface；经 Human Owner 明确批准，Minimal V2 先以 epoch 3 只发布 `deliveryTarget` 和交付安全链，同时保留全部 25 个工具。其余收敛继续保持有界发布，不倒推已完成。
**进入条件**：
1. Phase 7、Phase 8 完成；**已满足**。
2. `GG-BL-010` 已于 2026-08-30 由 Human Owner 接受剩余 observation 风险并关闭；**已满足**，但不等于平台根因被证明消除。
3. Minimal V2 的正式 contract release 已获得授权并完成 epoch 3 activation、Production App refresh 与 fresh-conversation read probe；**已满足**。
**一次性变更目标**：
- public `TaskBrief.deliveryTarget = local | pr | deploy` 正式进入 contract，并复用 Phase 8 已验证的内部 projection 语义；**已完成（Minimal V2）**；
...
**Minimal V2 closeout evidence**：production Gateway build `311d55f4a1651f7a560ca4c7f80371a74121b0bb`；25 tools / epoch 3 / digest `sha256:d5243888a58a440b05147d8e5baeb3713e92833720c5dd403901493ff555b496`；Pactify 七个任务 accepted；Gateway/Console deployment、Production App refresh 与 fresh-conversation `grande_task_status` read probe PASS。

**剩余退出条件**：完成上列仍未标记完成的工具合并/内部化，补齐 `GG-BL-024` 对应目标客户端和 rollback release 要求；不得把 Minimal V2 slice 的完成状态扩大为整个 Phase 9 DONE。
