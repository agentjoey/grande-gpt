...
- **Problem**: 当前 25-tool contract 中仍有 onboarding 两工具、capability inspect、deploy verify、task close 等可在保持风险语义的前提下合并或内部化。Minimal V2 已把 delivery-target projection 正式发布为 public schema，但其余工具若零散修改仍会放大 ChatGPT binding/snapshot 排障变量。
- **Evidence / Detail**: 2026-08-22 owner-approved simplification proposal 原计划一次正式 tool epoch 收敛；Phase 8 完成 no-tool-epoch primitives，2026-09-08 经明确授权的 Minimal V2 slice 使用 epoch 3 发布 `deliveryTarget` 并保留全部 25 个工具。`GG-BL-010` 已由 Human Owner 接受剩余风险并关闭，但其证据仍证明 session/app binding 与 server tool identity 可分叉，后续 contract release 必须继续走受控 refresh/new-chat 验证。
- **2026-09-08 Minimal V2 closeout（implemented and production-activated）**: Minimal V2 automatic terminal delivery 已完成 Gateway 实现、Pactify 七任务 acceptance、canonical merge、push 与 production activation。public contract 唯一 intentional delta 是 `grande_task_open` 可选且不可变的 `deliveryTarget` (`local | pr | deploy`)；正式 identity 为 **epoch 3 / toolsCount 25 / digest `sha256:d5243888a58a440b05147d8e5baeb3713e92833720c5dd403901493ff555b496`**，Gateway build 为 `git:311d55f4a1651f7a560ca4c7f80371a74121b0bb`。Human 只在受保护 Console 审批一次，approval/nonce/argv 不进入 public MCP；审批后 exact-bound merge/deploy/verify 由现有 GrandeGPT 工具可重入推进，`grande-gpt` 自身 DONE 另有 activation receipt 门禁。验证证据：focused 8 files / 101 tests、selfhost-safe 138 files / 1172 tests、tool-contract 2 files / 15 tests、Host 11 files / 195 tests、typecheck、Gateway/Console restart、公网 Access smoke、Production App refresh 与 fresh-conversation `grande_task_status` read probe 全部 PASS。done_when ③（删除工具收敛，本轮刻意保留全部 25 工具）、④（repo_register 收敛）、⑥（task_close 移除）及完整 rollback release 要求仍未完成，因此本条保持 `in-progress`。
- **Next**: 在不扰动已发布 epoch 3 Minimal V2 contract 的前提下，单独规划剩余 surface convergence：`repo_add_propose/apply → grande_repo_register`、capability inspect 并入 list filter、deploy verify 并入可重入 deploy、正常完成路径移除公开 task_close，以及剩余 client/rollback release evidence。
...
- **Related**: `GG-BL-001`（canonical refresh，已 DONE）、`GG-BL-008`（GrandeGPT GitHub credential least-privilege）、`GG-BL-017`（per-repo write lock，已 DONE）、`GG-BL-010`（owner-accepted residual-risk closeout，已 DONE）、`GG-BL-024`（剩余 Tool Surface Convergence）。
- **Blocked by / sequencing**: `GG-BL-010` 已由 Human Owner 接受剩余风险并关闭，Minimal V2 epoch 3 也已 activation；本项仍未获得独立实现授权。可以先设计/实现不暴露的新 internal primitive 与测试，但任何 public surface 变化仍应与 `GG-BL-024` 剩余收敛一次评审和发布，避免额外 tool snapshot/digest churn。
- **Original status**: BLOCKED
