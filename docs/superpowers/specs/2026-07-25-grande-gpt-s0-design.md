...
> **要求（S0-B 硬门禁，写工具随附）**：任何写工具在调用 `resolveInRepo` 之后，必须再过一道
...
**映射发生在 S0-D 的工具/信封层**，不是 S0-B。理由是所有异常在那里汇合成一个
`error{code,...}`，映射只该有一处。S0-B 与 S0-C 各自**只负责抛出带结构化 `.code`
的异常**，不互相知道对方的错误码 —— 这也让两个切片之间没有共享文件，可以并行开发。

映射表（不得靠解析 message 字符串）：
