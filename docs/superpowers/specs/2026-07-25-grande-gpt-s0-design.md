...
S0 本身仍是 13–19 人日的投入，而它押在一个**1–2 天就能验证的假设**上：GPT-5.6 在 chat
模式下究竟能不能撑住这种交互（尤其是**能否自主轮询**）。

因此在 S0 之前插入 **POC**：用九个返回硬编码假数据的工具搭一个假 MCP 服务端，不含 Gateway、
Seatbelt、worktree 与数据库，接上真实 ChatGPT 对话测五件事（验收标准见 §9.1）。

**POC 未通过则不启动 S0。** 详细论证见 §13。
...
- **任何形式的脚本化 / 无人值守驱动 ChatGPT**，或以规避额度为目的的自动化（见 §13.2 R4）
...
### 9.1 POC 验收标准（S0 的前置门禁）

**范围**：九个工具全部返回硬编码假数据。**不含** Gateway、Seatbelt、worktree、SQLite、
审计、OAuth 之外的任何真实逻辑。接 `m2m.agentjoey.ai`，在真实 ChatGPT 对话里跑一个模拟任务。

| # | 验证项 | 判定 | 不通过意味着 |
|---|---|---|---|
| **P-1** | **模型是否自主轮询** —— `grande_run` 返回后，无需用户催促即调用 `grande_run_result` 直至完成 | 连续 3 次模拟任务均自主完成轮询 | **最关键。** 异步模型需重新设计；`pollAfterSeconds` + `hint` 的引导手段不成立 |
| **P-2** | 一个模拟任务消耗多少条额度 | 记录 Sol / Terra / Luna 各自消耗 | 决定模型分工，甚至是否必须上 Pro |
| **P-3** | 连续 30 次工具调用后模型是否仍记得 `taskId` 与任务目标 | 第 30 次调用仍携带正确 `taskId` | `taskContext` 回带不足，需更激进的状态回灌 |
| **P-4** | 确认框实际打断次数；会话内「记住」是否真的生效 | 记录次数；确认「记住」后不再弹 | 工具粒度需重新划分 |
| **P-5** | 响应被标记 `truncated` 后，模型是否主动用 `nextCursor` 续读 | 3 次中至少 2 次主动续读 | 分页设计无效，需改为强制分块或在 `hint` 里更强引导 |

**P-1 是硬门禁**：不通过则暂停项目并重新设计交互模型，不得直接进入 S0。
P-2–P-5 不通过不阻塞，但结论必须写回本规格再启动 S0。

POC 的全部产出是**一份观察记录**（存 `docs/research/`）与对本规格的修订，**代码是一次性的，
不进入 S0 代码库**。
...
| 切片 | 内容 | 粗估（人日） |
|---|---|---|
| **POC** | **交互可行性验证：假 MCP 服务端 + 硬编码数据，验 P-1～P-5（§9.1）** | **1–2** |
...
| ~~Cloudflare Tunnel 的域名~~ | **已定：`m2m.agentjoey.ai`**（Cloudflare Tunnel 已有） | 已解决 |
| ChatGPT 账号的训练数据设置（D12） | 在 Data Controls 中关闭「用我的内容改进模型」 | 需你在账号里确认；或评估是否改用 Business 账号 |
...
> 本节是对整个项目方向的反向评估，写于 2026-07-25 设计定稿前。
> **它不是「已解决的风险清单」，而是「尚未证伪的怀疑」。** POC 的存在就是为了检验其中最致命的一条。

### 13.1 项目的真实动机及其脆弱性

本项目的直接动机是**利用 ChatGPT chat 模式的订阅额度访问前沿模型（GPT-5.6），
以替代按量计费的 API**。

这个动机本身是合理的，但它把整个项目建立在**一个第三方消费级产品的计费政策持续有利**
这一前提上。该前提不受我们控制，且可随时变化。因此：

**若项目价值 100% 来自成本套利，则不建议投入 55–85 人日。** 论证见 13.2 与 13.5。

### 13.2 五个关键风险

#### R1 · 额度并不「几乎免费」

报道中的 Plus 侧 GPT-5.6 配额（**非官方文档，需用实际账号核实**）：

| 模型 | Plus 配额 | 定位 |
|---|---|---|
| **Sol** | **15–90 条 / 5 小时** | 架构、复杂实现、困难调试 |
| Terra | 20–110 条 / 5 小时 | 常规实现、测试、重构 |
| Luna | 50–280 条 / 5 小时 | 机械修改、摘要 |
...
依赖一个**未经验证的前提**：模型能否在单个回合内自主等待并重复调用轮询工具。

| 情形 | 后果 |
|---|---|
| **能自主轮询** | 一条用户消息 → 模型串起 20 次工具调用 → 消耗 1 条额度。**方案成立** |
| **不能** | 每次轮询需用户手动催促 → 一个跑 3 分钟的测试烧掉 5–10 条额度 + 5–10 次人工介入。**Sol 的 15 条 /5h 连一个任务都跑不完，方案不成立** |
...
研究表明**性能退化远早于上下文窗口上限**：20 万 token 窗口的模型可在 5 万 token 处出现显著退化；
「lost-in-the-middle」使中段信息准确率下降 30% 以上。

| | Codex CLI / Claude Code | ChatGPT chat |
|---|---|---|
| 压缩机制 | 显式 compaction，用专门项**保留模型理解** | 早期对话被摘要压缩，**细节丢失**，优先保留近期消息 |
| 设计目标 | 长时 agentic 循环 | 人类对话 |

§5.5 的 `task_status` 跨会话恢复只能恢复**状态**，恢复不了**推理轨迹**（试过什么、为何放弃、
哪条是死胡同）。真实开发任务常需数十轮，该损耗持续存在。POC 的 P-3 部分检验此项。

#### R4 · ToS 与训练数据

**ToS**：OpenAI 消费者条款禁止「自动或程序化提取数据或 Output」、禁止「规避任何速率限制或
限制」，并限定「个人、非商业用途」。

判断：**GrandeGPT 的设计形态大概率合规** —— 有真人在对话中、逐次点确认框，使用的是 OpenAI
官方提供的 developer mode，这正是该功能的用途。**但边界清晰：一旦脚本化驱动、无人值守运行、
或为规避额度做任何自动化，即越线。** 已写入 §2.3 项目级永久非目标。

**训练数据（原设计缺口）**：Free 与 **Plus 消费者账号默认会用用户内容改进模型**，
除非在 Data Controls 中关闭；Business/Enterprise 与 API 的默认相反。
**私有代码会流经 ChatGPT 对话。** 处置见 D12。

#### R5 · 投入产出比

**Codex CLI 与 Claude Code 已原生提供此能力，且在纯技术维度上更优**：无 60s 超时、无逐次
确认框、有显式 compaction、有子代理与 hooks、无隧道跳数。

GrandeGPT 唯一的技术差异化是**访问 GPT-5.6 的计费方式**。

因此成本账为：**投入 55–85 人日工程以节省 API 费用。** 即便由 AI 大幅提效，仍是数月实际时间。
除非使用量极大，纯成本账算不过来。
...
1. **从移动端驱动本机开发。** ChatGPT app → 隧道 → 本机仓库，跑测试、看结果、改代码。
   ...
   ⚠️ **该价值尚未验证，且存在明确的证伪风险。** 官方 developer mode 文档只写
   「available … **on the web**」，全文未提移动端；二手来源（社区、Apps SDK 文档旧版）报告
   「MCP write actions are currently disabled on mobile devices」，只读可用。若属实，则手机上
   `grande_task_open` / `grande_repo_edit` / `grande_run` 三个写工具全部不可用 —— 能读代码，
   但开不了任务、改不了代码、跑不了测试，本条价值基本不成立。

   Human Owner 依据自有 NAS AI Ops 生产系统的经验判断移动端触发与确认可行；该系统确实
   包含写工具（`workspace_write` / `docker_action`），但其分享文档未涉及移动端，也无法确定
   它走的是否同一接入面。**因此列为 POC 的 P-6 实测项**（见 `poc/PROTOCOL.md` §1.5）。

   **若 P-6 证实移动端限制写操作**，降级路线已有现成先例：参考项目的**确认字符串模式**
   （危险操作必须携带匹配的显式确认串）。据此可把写操作拆成「移动端出意图 + 确认串」与
   「另一侧落地执行」两段——移动端只需只读工具即可参与，写权限不必存在于移动会话中。
   该方案在 P-6 出结果前不实现。
...
3. **Gateway 不绑定 ChatGPT。** 它是标准 MCP 服务端，Claude、Cursor 及任何 MCP 客户端均可接入。
   这是可复用资产，而非为 ChatGPT 定制的一次性投入。

### 13.4 项目目标的重新表述

基于 13.3，本项目的目标应表述为：

> **建立一个受控的、多客户端可接入的本地开发控制平面；ChatGPT chat 是它的第一个前端。**

而非「用 ChatGPT 省 API 钱」。前者支撑 55–85 人日的投入，后者不支撑。

本规格其余部分的设计**不因这一重述而改变** —— 边界、工具集、沙箱策略在两种表述下都成立。
改变的是**取舍时的判断依据**：当某项设计在「对 ChatGPT 更方便」与「对任意 MCP 客户端更通用」
之间冲突时，选后者。
...
| 若你的目标是 | 结论 |
|---|---|
| 纯成本套利 | **不建议**投入 55–85 人日。R1 + R5 使账算不过来，R4 增加不确定性 |
| 移动端可达 + 受控开发平台 + 可复用 Gateway | **建议做**，但必须先过 POC |

**无论哪种情形，POC 都应先做** —— 1–2 人日即可把最致命的 R2 从「假设」变成「证据」。

### 13.6 本节的复审时机

以下事件发生时必须重读并修订本节：

- POC 完成后（P-1～P-5 的实测结果直接影响 R2、R3 的严重度评估）
- OpenAI 调整 ChatGPT 套餐额度或消费者条款
- 出现 MCP Tasks 扩展在 ChatGPT 落地（可根本性改善 R2）
- S0 完成后依 AC-13 复审工具设计时（见 §12.3）
...
| 首个里程碑 | v0.1 完整本地开发（~30 人日） | **POC 交互验证（1–2 人日）**，通过后才做 S0 | 产品风险大于工程风险，且最致命的假设 1–2 天可测（§13.2 R2） |
| 控制台 | 未列入 MVP | S2.5，T3 流程 | 用户要求 |
| 方向质疑 | 无 | §13 方案风险与前提质疑 | 草案默认方向成立；本规格记录了「若目标仅为省钱则不建议做」的论证 |
| 训练数据 | 未提及 | D12 + §13.2 R4 | Plus 消费者账号默认用内容训练，对代码平台是实质问题 |
...
§13 引用的外部资料（访问日期 2026-07-25）：

- [OpenAI 消费者使用条款](https://openai.com/policies/row-terms-of-use/) —— 程序化提取、规避速率限制、个人非商业用途的限制
- [ChatGPT 额度报道](https://tokenkarma.app/blog/openai-rate-limits-july-2026/) —— **第三方报道，非官方文档，需实测核实**
- [How Long Contexts Fail](https://www.dbreunig.com/2025/06/22/how-contexts-fail-and-how-to-fix-them.html) —— 长上下文失效模式
- [Context Rot](https://www.producttalk.org/context-rot/) —— 退化早于窗口上限、lost-in-the-middle
